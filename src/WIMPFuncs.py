...
def WIMPDiffRate(E_th,E_max,ne,m_vals,Nuc,NuclearEnhancementFactor,FormFactor,gvmin,**kwargs):
    EmaxSet = np.array(list(map(lambda m: MaxWIMPEnergy(Nuc.MassNumber,m,**kwargs),m_vals)))
    massSet= m_vals[EmaxSet>(2*E_th)]
    E_be = logspace(log10(E_th),log10(E_max),ne+1)
    nm = size(massSet)
    R = zeros(shape=(nm,ne))
    E_r_max = MaxWIMPEnergy(Nuc.MassNumber,massSet[0],**kwargs)
    Efine = logspace(log10(E_th),log10(E_r_max),1000)
    return [Efine,dRdE(Efine,massSet[0],1.0e-45,Nuc,NuclearEnhancementFactor,FormFactor,gvmin,**kwargs)]

def U1BinnedWIMPRate(E_th,E_max,ne,m_vals,Nuc,NuclearEnhancementFactor,FormFactor,gvmin,v_lab=245.6,**kwargs):
    EmaxSet = np.array(list(map(lambda m: MaxWIMPEnergy(Nuc.MassNumber,m,v_lab=v_lab,**kwargs),m_vals)))
    massSet= m_vals[EmaxSet>(2*E_th)]
    #E_be = logspace(log10(E_th),log10(E_max),ne+1)
    ...
    R = zeros(shape=(nm,ne))
    for i in range(0,nm):
        E_r_max = MaxWIMPEnergy(Nuc.MassNumber,massSet[i],v_lab=v_lab,**kwargs)
        Efine = np.linspace(E_th,E_r_max,1000)
        R_tot = np.trapz(dRdE(Efine,massSet[i],1.0e-45,Nuc,NuclearEnhancementFactor,FormFactor,gvmin,**kwargs),Efine)
        #print(R_tot)
        dR = dRdE(E_be,massSet[i],1.0e-45,Nuc,NuclearEnhancementFactor,FormFactor,gvmin,**kwargs)
        R[i,:] = 0.5*(E_be[1:]-E_be[0:-1])*(dR[1:]+dR[0:-1])
        R[i,:] = R_tot*R[i,:]/sum(R[i,:])
